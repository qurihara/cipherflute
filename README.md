...
音の高さが読み取れれば、その並びが記号になる。何を書き込むかは、どの笛を
どの順で並べるかで決める。
...
| `gallery/` | 作例の写真と造形データ（かるた札ペアキーホルダー・ペアカード・カード・箱・画像タイル・照合笛） |

## 笛の素材（`flutes/`）

スロット（音の高さの置き場）は **G#6 から G7 までの12個**、半音刻みである。半音刻みにしてあるので、
平均律の鍵盤楽器がそのまま対照表になる。

**外形はすべて同じ 65.97 × 7 × 4mm で、中の空洞の長さだけが違う。**
見ただけでは、どの笛がどの音かは分からない。

管の長さと音の高さは `calibration.json` の式で対応づける。
...
鳴る（360度のうち270度）。`embed/orient_check.py` で確認できる。
...
`codec/verify_manual_decode.py` で確認できる。

## 復号器（`decoder/`）

ブラウザだけで動く。その場で吹いても、録音した音声を入力しても復号できる。
...
音の高さは**笛の音域の中でいちばん強い山を追う**方式で検出する（`fft_peak.js`）。
...
判断を伴う部分は手順書の側にあるので、埋め込みの再現には手順書とプログラムの両方を必要とする。

**核心は、笛の外形の凸包でホスト側にポケットを彫り抜いてから笛を戻すことである。**
これを略すと、ホストの材料が笛の空洞を埋めて鳴らなくなる。

作ったものが鳴るかどうかは、印刷する前に2つの検査で確認できる。
